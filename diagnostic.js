    ...
    feel: {
        Engine: {
            ...
            potentialIssues: [
                ...
                "Transmission or Engine Problem. In automatic transmission, the engine can stall if there are issues with fluid levels, pressure, or internal components. The transmission fluid could be dirty or low, affecting the transmission to engage gear properly. In manual, it is most likely related to clutch problems, being worn out or issues with the clutch linkage leading to a stall. For both automatic and manual, the engine may stall due to improper air, fuel, or ignition delivery.",
...
// Function to highlight important words in text
function highlightText(text) {
    const importantWords = ["Inspect", "Check", "Replace", "Adjust", "Fluid", "Battery", "Make sure", "Look", "Test", "Tighten", "Locate"];
    importantWords.forEach(word => {
        const regex = new RegExp(`\\b(${word})\\b`, 'g'); // Case-sensitive match
        text = text.replace(regex, `<mark>${word}</mark>`);
    });
    return text;
}

// Display potential issues and diagnostic actions with highlights
function displayPotentialIssueAndAction(data) {
    ...
    potentialIssueBubble.innerHTML = `<img src="newlogo.png" alt="AG Tech Bot" class="chatbot-logo"><div class='chat-message'><mark>Potential Issue:</mark> ${highlightText(data.potentialIssue)}</div>`;
    ...
    setTimeout(() => {
        ...
        diagnosticActionBubble.innerHTML = `<img src="newlogo.png" alt="AG Tech Bot" class="chatbot-logo"><div class='chat-message'><mark>Diagnostic Action:</mark> ${highlightText(data.diagnosticAction)}</div>`;
