...
try {
    ...
    // SQL query to fetch all appointments and join with admin_appointments
    $sql = "
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        // Reformat data to match the expected structure with separate vehicle fields
        $appointments[] = [
            'date' => $row['appointment_date'],
            'name' => $row['first_name'] . ' ' . $row['last_name'],  // Combine first and last name
            'time' => $row['appointment_time'],
            'service' => $row['services'],
            'car_brand' => $row['car_brand'], // Store car brand separately
            'car_model' => $row['car_model'], // Store car model separately
            'car_year' => $row['car_year'],   // Store car year separately
